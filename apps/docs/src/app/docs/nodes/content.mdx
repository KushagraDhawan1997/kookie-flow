...
<KookieFlow
  ...
  header="outside"   // 'inside' (default) | 'outside' | 'none' (no title)
  accentHeader       // Thin accent light along the top edge, whatever the header position
/>
```

### Per-Entity Colors

Each entity can have its own accent color from Kookie UI's 26-color palette. It draws as a thin
light along the entity's top edge, in that hue — the same band `accentHeader` draws in the theme's
accent:
