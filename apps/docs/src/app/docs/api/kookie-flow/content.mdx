...
| `header` | `HeaderPosition` | `'inside'` | Where the entity's title is drawn: `'inside'` the body, `'outside'` above it, or `'none'` for no title at all |
| `accentHeader` | `boolean` | `false` | Draw a thin accent light along every entity's top edge (any header position) |
