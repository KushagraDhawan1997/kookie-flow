...
function useHistory(maxHistory = 50) {
  ...
  const undo = useCallback(() => {
    ...
    // Restore state
    store.getState().setEntities(previous.entities);
  ...
  const redo = useCallback(() => {
    ...
    // Restore state
    store.getState().setEntities(next.entities);
...
5. **Know what a snapshot leaves out** - `toObject()` returns `entities`, `edges` and `viewport`. It does not capture stacking order (which node is in front, set by the last press) or any widget value the person has set but your `onEntitiesChange` handler has not echoed back onto the entity yet. Undoing restores what the entities say; a node that was brought to the front stays in front, and an un-echoed in-flight value is lost rather than restored.
