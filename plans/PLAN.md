...
flowRef.current.setSocketValue(entityId, socketId, value);  // inject value programmatically
flowRef.current.getSocketValue(entityId, socketId);         // read current value
```
...
| Phase | Status set by library | Visual (GPU-rendered, already implemented) |
|---|---|---|
| Inputs changed, not yet evaluated | `dirty` | Subtle stale indicator (new — to be designed) |
| `onEvaluate` called | `running` | Pulsing accent border animation |
| `onEvaluate` returns | `success` | Brief green flash, then clears |
| `onEvaluate` throws | `error` + `statusMessage` | Red border, persists until re-evaluated |
| Evaluation cancelled (inputs changed) | Back to `dirty` | Stale indicator |

Consumer can override or supplement via callback:
...
| **Dirty tracking** | Widget change, connection change, or upstream output change → mark dirty + downstream dirty. |
...
| **Widget visibility** | Input socket connected → hide widget (value comes from edge). Disconnected → show widget. |
| **Preview updates** | Output socket value arrives → preview re-renders with new data. |
| **Muted nodes** | Muted entities skipped in evaluation. Inputs pass through to outputs (bypass). |
