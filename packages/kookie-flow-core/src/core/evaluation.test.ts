...
import type { Edge, Entity, Socket } from '../types/index';
...
function edge(source: string, sourceSocket: string, target: string, targetSocket: string): Edge {
  return {
    id: `${source}.${sourceSocket}->${target}.${targetSocket}`,
    source,
    sourceSocket,
    target,
    targetSocket,
...
describe('incremental scheduling under load', () => {
  it('keeps a long chain linear in entity lookups and produces every result', async () => {
    ...
    const entities = Array.from({ length: n }, (_, i) =>
      ent(String(i), [sock('in', 0)], [sock('out')])
    );
  ...
  it('handles wide fan-out, reconvergence and two sockets from one predecessor', async () => {
    ...
    const w = world(
      [ent('root', [], [sock('out')]), ...leaves, ent('join', inputs, [sock('out')])],
      edges
    );
    ...
    const ev = new Evaluator(hostFor(w), async (id, _, values) => {
      ...
      if (id === 'join')
        return { out: Object.values(values).reduce<number>((sum, v) => sum + Number(v), 0) };
  ...
  it('recomputes readiness when an error-blocked target is rewired to a settled source', async () => {
    const w = world(
      [
        ent('bad', [], [sock('out')]),
        ent('good', [], [sock('out')]),
        ent('target', [sock('in')], [sock('out')]),
      ],
      [edge('bad', 'out', 'target', 'in')]
    );
  ...
  it('matches an independent topological reference on a deterministic mixed DAG', async () => {
    ...
    for (let i = 0; i < count; i++) {
      const parents = [...new Set([i - 1, Math.floor(i / 2), (i * 17) % Math.max(1, i)])].filter(
        (p) => p >= 0 && p < i
      );
      entities.push(
        ent(
          String(i),
          parents.map((p) => sock(`from${p}`)),
          [sock('out')]
        )
      );
    ...
    const w = world(entities, edges);
    const ev = new Evaluator(hostFor(w), (_, __, values) => ({
      out: (1 + Object.values(values).reduce<number>((sum, v) => sum + Number(v), 0)) % 100003,
    }));
    try {
      await ev.evaluateAll();
...
describe('manual gates', () => {
  it('a manual entity is marked dirty by upstream change but does not run', async () => {
    const w = world(
      [
        ent('a', [], [sock('o')]),
        ent('gen', [sock('i')], [sock('o')], 'ai/generate'),
        ent('post', [sock('i')]),
      ],
...
describe('cancellation', () => {
  ...
  it('a late rejection from a superseded run does not mark the entity as failed', async () => {
    ...
    const ev = new Evaluator(hostFor(w), () => {
      n++;
      if (n === 1)
        return new Promise<Values>((_r, rej) => {
          rejectFirst = rej;
        });
      return { o: 'ok' };
    });
...
describe('status lifecycle', () => {
  ...
  it('stamps each transition with when it happened, so a renderer can animate a state', async () => {
    const w = world([ent('a', [sock('i', 1)])]);
    const ev = new Evaluator(hostFor(w), async () => {
      await tick();
      return {};
    });
    ev.markDirty('a');
...
describe('the shapes that break naive engines', () => {
  ...
  it('a synchronous onEvaluate works the same as an async one', async () => {
    const w = world(
      [ent('a', [sock('i', 3)], [sock('o')]), ent('b', [sock('i')])],
      [edge('a', 'o', 'b', 'i')]
    );
    let bSaw: Values = {};
    const ev = new Evaluator(hostFor(w), (id, _t, inputs) => {
  ...
  it('hands the entity itself to onEvaluate, so a consumer need not look it up', async () => {
    ...
    const ev = new Evaluator(hostFor(w), (_id, _t, _in, ctx) => {
      seen = ctx.entity;
      return {};
    });
    ev.markDirty('a');
  ...
  it('onEvaluate may inject values into another entity without breaking the pass', async () => {
    ...
    const w = world(
      [ent('a', [sock('i', 1)]), ent('x', [], [sock('o')]), ent('y', [sock('i')])],
      [edge('x', 'o', 'y', 'i')]
    );
    const ran: string[] = [];
    let ySaw: Values = {};
...
describe('injected values and the whole graph', () => {
  ...
  it('dispose aborts everything in flight', async () => {
    ...
    const ev = new Evaluator(hostFor(w), (_i, _t, _in, ctx) => {
      signal = ctx.signal;
      return new Promise(() => {});
    });
    ev.markDirty('a');
