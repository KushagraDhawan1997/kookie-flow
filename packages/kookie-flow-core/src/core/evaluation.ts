...
import type { Entity, Socket } from '../types/index';
...
export class Evaluator {
  ...
  markDirty(ids: string | readonly string[]): void {
    ...
    while (stack.length > base) {
      ...
      if (!entity || !isEvaluated(entity.type)) {
        ...
        if (ports)
          for (const edges of ports.values()) for (const edge of edges) stack.push(edge.target);
        changed = true;
        continue;
      }
  ...
  private writeOutput(entityId: string, socketId: string, value: unknown): void {
    ...
    if (!keys) {
      keys = new Set();
      this.outputKeys.set(entityId, keys);
    }
    keys.add(key);
  ...
  private propagate(id: string): void {
    ...
    if (ports)
      for (const edges of ports.values()) for (const edge of edges) this.markDirty(edge.target);
    this.schedule();
  }
