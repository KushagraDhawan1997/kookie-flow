...
export function getEdgesBetween(index: AdjacencyIndex, entityA: string, entityB: string): Edge[] {
...
export function* walkUpstream(index: AdjacencyIndex, startEntityId: string): Generator<string> {
...
export function* walkDownstream(index: AdjacencyIndex, startEntityId: string): Generator<string> {
  const visited = new Set<string>();
  visited.add(startEntityId);
  const stack: string[] = [];
  ...
  for (const root of entityIds) {
    ...
    while (stack.length) {
      ...
      if (ports)
        for (const edges of ports.values())
          for (const edge of edges) {
            if (live.has(edge.target) && !seen.has(edge.target))
...
export function getAffectedEntities(
  ...
  if (cachedOrder) {
    ...
    return Array.from(affected).sort((a, b) => (orderIndex.get(a) ?? 0) - (orderIndex.get(b) ?? 0));
  }
...
export function areConnected(index: AdjacencyIndex, entityA: string, entityB: string): boolean {
 ...
 */
export function getExecutionOrder(index: AdjacencyIndex, targetEntityId: string): string[] {
  ...
  const validSource = (edge: Edge) => {
    ...
    return (
      ...
      (edge.sourceSocket === undefined ||
        !!source.outputs?.some((socket) => socket.id === edge.sourceSocket))
    );
  };
  ...
  if (socketTypes || isTypeCompatible) {
    for (const edge of edges) {
      ...
      if (isTypeCompatible) {
        ...
        compatible = defaultTypeCompatible(sourceSocket.type, targetSocket.type, socketTypes);
      }
 ...
 */
export function isGraphComplete(entities: Entity[], index: AdjacencyIndex): boolean {
  for (const entity of entities) {
    if (!entity.inputs) continue;
    for (const input of entity.inputs) {
