...
export class Quadtree {
  ...
  private queryRangeInternal(range: Bounds, results: string[], seen: Set<string>): void {
  ...
  private subdivide(): void {
    ...
    this.nw = new Quadtree({ x, y, width: halfW, height: halfH }, this.capacity, this.depth + 1);
 ...
 */
export function boundsFromCorners(x1: number, y1: number, x2: number, y2: number): Bounds {
  return {
    x: Math.min(x1, x2),
    y: Math.min(y1, y2),
...
export class SocketQuadtree {
  ...
  private subdivide(): void {
    ...
    this.nw = new SocketQuadtree(
      { x, y, width: halfW, height: halfH },
      this.capacity,
      this.depth + 1
    );
    this.ne = new SocketQuadtree(
      { x: x + halfW, y, width: halfW, height: halfH },
      this.capacity,
      this.depth + 1
    );
    this.sw = new SocketQuadtree(
      { x, y: y + halfH, width: halfW, height: halfH },
      this.capacity,
      this.depth + 1
    );
    this.se = new SocketQuadtree(
      { x: x + halfW, y: y + halfH, width: halfW, height: halfH },
      this.capacity,
      this.depth + 1
    );
