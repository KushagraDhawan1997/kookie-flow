...
const rect = (id: string, x: number, y: number, width = 100, height = 50): ArrangeRect => ({
  id,
  x,
  y,
  width,
...
describe('alignRects', () => {
  ...
  it('left, right and centre line up on the bounds of the selection itself', () => {
    ...
    expect(apply(rects, alignRects(rects, 'right')).map((r) => r.x + r.width)).toEqual([
      250, 250, 250,
    ]);
    expect(apply(rects, alignRects(rects, 'center')).map((r) => r.x + r.width / 2)).toEqual([
      125, 125, 125,
    ]);
  });
  ...
  it('top, bottom and middle do the same on the other axis, and never move x', () => {
    ...
    expect(apply(rects, alignRects(rects, 'bottom')).map((r) => r.y + r.height)).toEqual([
      320, 320, 320,
    ]);
    expect(apply(rects, alignRects(rects, 'middle')).map((r) => r.y + r.height / 2)).toEqual([
      160, 160, 160,
    ]);
  });
