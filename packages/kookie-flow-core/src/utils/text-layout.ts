...
import { hexToRGB } from './hex-color';
 ...
 */
export function measureText(text: string, glyphMap: GlyphMap, kerningMap: KerningMap): number {
  let width = 0;
 ...
 */
export function countMultiLineGlyphs(entries: MultiLineTextEntry[], glyphMap: GlyphMap): number {
  let count = 0;
  for (const entry of entries) {
