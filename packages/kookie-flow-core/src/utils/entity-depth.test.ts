...
describe('entityDepth', () => {
  ...
  it('stays inside the camera range for a graph of any size', () => {
    ...
    for (const n of [2000, 2500, 3000, 5000, 20000]) {
      ...
      expect(entityDepth('e1', order, new Set(['e1']))).toBeGreaterThan(
        entityDepth(top, order, none, DEPTH_LAYER.label)
      );
      expect(entityDepth(top, order, none)).toBeGreaterThan(
        entityDepth(`e${n - 1}`, order, none, DEPTH_LAYER.label)
      );
    }
  });
  ...
  it('is unchanged below the compaction point', () => {
    ...
    expect(entityDepth('a', order, none, DEPTH_LAYER.widget)).toBeCloseTo(
      -850 + 3 * STACK_STEP + DEPTH_LAYER.widget
    );
  });
