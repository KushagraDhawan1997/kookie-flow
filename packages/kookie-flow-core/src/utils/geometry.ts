...
import type { Entity, Edge, XYPosition, Viewport, SocketHandle, EdgeType } from '../types/index';
import {
 ...
 */
export function screenToWorld(screenPos: XYPosition, viewport: Viewport): XYPosition {
  return {
 ...
 */
export function worldToScreen(worldPos: XYPosition, viewport: Viewport): XYPosition {
  return {
...
export type SocketIndexMap = Map<
  string,
  { index: number; socket: { id: string; type: string; position?: number } }
>;
...
export function getEdgeAtPosition(
  ...
  for (const edge of edges) {
    ...
    const sourceYOffset = calculateSocketYOffset(
      sourceEntity,
      edge.sourceSocket,
      false,
      socketIndexMap,
    ...
    const targetYOffset = calculateSocketYOffset(
      targetEntity,
      edge.targetSocket,
      true,
      socketIndexMap,
    ...
    const x0 = sourceIsReroute
      ? sourceEntity.position.x
      : getSocketWorldX(sourceEntity, false) + sourceRim;
    const y0 = sourceEntity.position.y + (sourceIsReroute ? 0 : sourceYOffset);
    const x1 = targetIsReroute
      ? targetEntity.position.x
      : getSocketWorldX(targetEntity, true) - targetRim;
...
export function getEdgePointAtT(
  ...
  const x0 =
    sourceEntity.type === 'reroute'
      ? sourceEntity.position.x
      : getSocketWorldX(sourceEntity, false);
  const y0 = sourceEntity.position.y + (sourceEntity.type === 'reroute' ? 0 : sourceYOffset);
  const x1 =
    targetEntity.type === 'reroute' ? targetEntity.position.x : getSocketWorldX(targetEntity, true);
