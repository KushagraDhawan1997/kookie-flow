...
import { useResolvedStyle } from '../contexts/index';
...
import type { VideoEntityData, EntityChange } from '../types/index';
...
export function VideoEntities({ onEntitiesChange }: VideoEntitiesProps) {
  ...
  const [videoEntityIds, setVideoEntityIds] = useState<string[]>(() =>
    store
      .getState()
      .entities.filter((e) => e.type === 'video')
      .map((e) => e.id)
  );
  ...
  const glass = useMemo(
    () =>
      buildMediaGlass(
        tokens,
        resolveColor(THEME_COLORS.canvas.background, tokens),
  ...
  useEffect(() => {
    registerVideoOps(store, 'entity', {
      toggle: (entityId) => {
        const src = (store.getState().entityMap.get(entityId)?.data as VideoEntityData | undefined)
          ?.src;
        if (!src) return;
  ...
  useFrame(({ size }, delta) => {
    ...
    for (let i = 0; i < ids.length; i++) {
      ...
      const wants = data.playing !== undefined ? data.playing : (asked ?? data.autoplay ?? false);
      if (src) {
      ...
      if (texture) {
        ...
        if (mat) {
          ...
          const played =
            element && duration && Number.isFinite(duration) && duration > 0
      ...
      if (
        ...
        entry.naturalWidth > 0 &&
        entry.naturalHeight > 0 &&
        !autoSizedRef.current.has(entity.id)
      ) {
