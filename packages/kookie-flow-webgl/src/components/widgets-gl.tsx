...
import {
  ...
  readWidgetValue,
  widgetKey,
} from '@kushagradhawan/kookie-flow-core/internal/utils/widget-values';
...
import type { SocketType, ResolvedWidgetConfig, WidgetType } from '../types/index';
...
export function WidgetsGL({
  ...
  const material = useMemo(() => {
    ...
    return new THREE.ShaderMaterial({
      uniforms: {
        ...
        uFocusRing: {
          value: new THREE.Vector3(FOCUS_RING.width, FOCUS_RING.offset, FOCUS_RING.land),
        },
        uTime: { value: 0 },
        uMotion: { value: 1 },
        uColourDur: {
          value: new THREE.Vector4(MOTION.hoverIn, MOTION.hoverOut, MOTION.mark, MOTION.ring),
        },
  ...
  useEffect(() => {
    ...
    const unsubs = [
      store.subscribe(
        (s) => s.hoveredWidget,
        (next, prev) => {
          restate(handleKey(prev));
          restate(handleKey(next));
        }
      ),
      store.subscribe(
        (s) => s.editingWidgetKey,
        (next, prev) => {
          restate(prev);
          restate(next);
        }
  ...
  useFrame(({ size }) => {
    ...
    const moving =
      colourMoving || pressMoving || ringMoving || checkMoving || switchMoving || travelMoving;
    ...
    for (let vi = 0; vi < visibleCount; vi++) {
      ...
      for (let i = 0; i < inputs.length; i++) {
        ...
        if (config.type === 'color') {
          ...
          buffers.tint[n * 3] =
            config.type === 'segmented'
              ? (config.options?.length ?? 0)
              : config.type === 'vector'
                ? vectorDimensions(config)
                : 0;
          buffers.tint[n * 3 + 1] =
            config.type === 'segmented' ? segmentIndex(config.options, value) : 0;
        ...
        if (config.type === 'segmented') {
          ...
          if (!m) {
            ...
            if (chosen >= 0 && m.to >= 0) {
              ...
              const l = travelEdge(
                ...
                m.to,
                m.start,
                leftLeads ? MOTION.travelLead : MOTION.travelTrail,
                now,
                reduced
              );
              const r = travelEdge(
                ...
                m.to + 1,
                m.start,
                leftLeads ? MOTION.travelTrail : MOTION.travelLead,
                now,
                reduced
              );
              m.fromL = l;
          ...
          const checked =
            buffers.kind[n] === KIND.checkboxOn || buffers.kind[n] === KIND.switchOn ? 1 : 0;
          const checkT =
            config.type === 'checkbox'
              ? checkTrack.read(key)
              : config.type === 'switch'
                ? switchTrack.read(key)
                : undefined;
    ...
    for (const [mesh, buffers, count] of [
      ...
      if (buffers.sizeAttr) {
        buffers.sizeAttr.addUpdateRange(0, count * 2);
        buffers.sizeAttr.needsUpdate = true;
      }
      if (buffers.radiusAttr) {
        buffers.radiusAttr.addUpdateRange(0, count);
        buffers.radiusAttr.needsUpdate = true;
      }
      if (buffers.kindAttr) {
        buffers.kindAttr.addUpdateRange(0, count);
        buffers.kindAttr.needsUpdate = true;
      }
      if (buffers.valueAttr) {
        buffers.valueAttr.addUpdateRange(0, count);
        buffers.valueAttr.needsUpdate = true;
      }
      if (buffers.tintAttr) {
        buffers.tintAttr.addUpdateRange(0, count * 3);
        buffers.tintAttr.needsUpdate = true;
      }
      ...
      if (buffers.hoverAttr) {
        buffers.hoverAttr.addUpdateRange(0, count);
        buffers.hoverAttr.needsUpdate = true;
      }
      if (buffers.animAttr) {
        buffers.animAttr.addUpdateRange(0, count * 4);
        buffers.animAttr.needsUpdate = true;
      }
      if (buffers.motionAttr) {
        buffers.motionAttr.addUpdateRange(0, count * 3);
        buffers.motionAttr.needsUpdate = true;
      }
      if (buffers.ringAttr) {
        buffers.ringAttr.addUpdateRange(0, count * 2);
