...
export function RerouteNodes() {
  ...
  const [rerouteEntityIds, setRerouteEntityIds] = useState<string[]>(() => {
    return store
      .getState()
      .entities.filter((e) => e.type === 'reroute')
      .map((e) => e.id);
  });
  ...
  const geometry = useMemo(
    () => new THREE.CircleGeometry(REROUTE_RADIUS, REROUTE_SEGMENTS),
    [capacity]
  );
  ...
  useFrame(({ size }) => {
    ...
    const { entityMap, viewport, hoveredEntityId, selectedEntityIds, hiddenEntityIds } =
      store.getState();
