...
import type { TextEntityData } from '../types/index';
import { DEFAULT_TEXT_SIZING_MODE } from '@kushagradhawan/kookie-flow-core/internal/core/constants';
...
export function TextEditCursor() {
  ...
  const cursorMaterial = useMemo(() => {
    return new THREE.ShaderMaterial({
      uniforms: {
        uColor: {
          value: new THREE.Color(primaryTextColor[0], primaryTextColor[1], primaryTextColor[2]),
        },
        uOpacity: { value: 1.0 },
      },
  ...
  useFrame((_state, delta) => {
    ...
    const { editingEntityId, editingContent, editingCursor, entityMap } = store.getState();
    ...
    if (hasSelection) {
      ...
      const selChanged =
        editingCursor.start !== sk.start || editingCursor.end !== sk.end || tableChanged;

      if (selChanged) {
        cachedRectsRef.current = getSelectionRects(editingCursor.start, editingCursor.end, table);
        ...
        for (let i = 0; i < count; i++) {
          ...
          matrix.setPosition(rect.x + rect.width / 2, -(rect.y + rect.height / 2), SELECTION_Z);
