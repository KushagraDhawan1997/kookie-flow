import { connectedSocketKey } from '@kushagradhawan/kookie-flow-core/internal/utils/socket-key';
...
import { areTypesCompatible } from '@kushagradhawan/kookie-flow-core/internal/utils/connections';
import type { Magnet } from '../gl/index';
import { THEME_COLORS, resolveColor } from '../core/theme-colors';
import type { Edge, Entity, SocketType } from '../types/index';
import { rgbToHex } from '../utils/color';
import {
  entityDepth,
  DEPTH_LAYER,
} from '@kushagradhawan/kookie-flow-core/internal/utils/entity-depth';
...
function markSocketBuffersForUpload(bufs: SocketBuffers, count: number) {
  ...
  if (count <= 0) return;
  if (bufs.colorAttr) {
    bufs.colorAttr.clearUpdateRanges();
    bufs.colorAttr.addUpdateRange(0, count * 3);
    bufs.colorAttr.needsUpdate = true;
  }
  if (bufs.hoveredAttr) {
    bufs.hoveredAttr.clearUpdateRanges();
    bufs.hoveredAttr.addUpdateRange(0, count);
    bufs.hoveredAttr.needsUpdate = true;
  }
  if (bufs.connectedAttr) {
    bufs.connectedAttr.clearUpdateRanges();
    bufs.connectedAttr.addUpdateRange(0, count);
    bufs.connectedAttr.needsUpdate = true;
  }
  if (bufs.validTargetAttr) {
    bufs.validTargetAttr.clearUpdateRanges();
    bufs.validTargetAttr.addUpdateRange(0, count);
    bufs.validTargetAttr.needsUpdate = true;
  }
  if (bufs.invalidHoverAttr) {
    bufs.invalidHoverAttr.clearUpdateRanges();
    bufs.invalidHoverAttr.addUpdateRange(0, count);
    bufs.invalidHoverAttr.needsUpdate = true;
  }
  if (bufs.magnetAttr) {
    bufs.magnetAttr.clearUpdateRanges();
    bufs.magnetAttr.addUpdateRange(0, count * 3);
    bufs.magnetAttr.needsUpdate = true;
  }
  if (bufs.layerAttr) {
    bufs.layerAttr.clearUpdateRanges();
    bufs.layerAttr.addUpdateRange(0, count);
    bufs.layerAttr.needsUpdate = true;
  }
}
...
function markSocketRangeForUpload(bufs: SocketBuffers, start: number, count: number) {
  if (count <= 0) return;
  if (bufs.colorAttr) {
    bufs.colorAttr.addUpdateRange(start * 3, count * 3);
    bufs.colorAttr.needsUpdate = true;
  }
  if (bufs.hoveredAttr) {
    bufs.hoveredAttr.addUpdateRange(start, count);
    bufs.hoveredAttr.needsUpdate = true;
  }
  if (bufs.connectedAttr) {
    bufs.connectedAttr.addUpdateRange(start, count);
    bufs.connectedAttr.needsUpdate = true;
  }
  if (bufs.validTargetAttr) {
    bufs.validTargetAttr.addUpdateRange(start, count);
    bufs.validTargetAttr.needsUpdate = true;
  }
  if (bufs.invalidHoverAttr) {
    bufs.invalidHoverAttr.addUpdateRange(start, count);
    bufs.invalidHoverAttr.needsUpdate = true;
  }
  if (bufs.magnetAttr) {
    bufs.magnetAttr.addUpdateRange(start * 3, count * 3);
    bufs.magnetAttr.needsUpdate = true;
  }
}
...
export function Sockets({ socketTypes = DEFAULT_SOCKET_TYPES }: SocketsProps) {
  ...
  const bgMaterial = useMemo(
    () =>
      new THREE.ShaderMaterial({
        uniforms: {
          uInvalidColor: {
            value: new THREE.Color(invalidColor[0], invalidColor[1], invalidColor[2]),
          },
          uValidTargetColor: {
            value: new THREE.Color(validTargetColor[0], validTargetColor[1], validTargetColor[2]),
          },
          ...
          uPointerOn: { value: 0 },
          uReach: {
            value: new THREE.Vector3(SOCKET_POINTER_REACH, SOCKET_POINTER_FUSE, SOCKET_LEAN),
          },
        },
        vertexShader,
        fragmentShader,
  ...
  const writeEntitySockets = (
    ...
    if (entity.inputs) {
      for (let i = 0; i < entity.inputs.length; i++) {
        ...
        tempMatrix.identity();
        tempMatrix.setPosition(getSocketWorldX(entity, true), -(entity.position.y + yOffset), z);
        ...
        if (connectionDraft && !connectionDraft.source.isInput && sourceSocketType) {
          ...
          const isTypeCompatible = areTypesCompatible(sourceSocketType, socket.type, socketTypes);
          isValidTarget = isStructurallyValid && isTypeCompatible ? 1.0 : 0.0;
        }
        bufs.validTarget[idx] = isValidTarget;

        const isInvalidHover = isHovered && connectionDraft && isValidTarget === 0.0 ? 1.0 : 0.0;
    ...
    if (entity.outputs) {
      for (let i = 0; i < entity.outputs.length; i++) {
        ...
        tempMatrix.identity();
        tempMatrix.setPosition(getSocketWorldX(entity, false), -(entity.position.y + yOffset), z);
        tempMatrix.toArray(matrixArray, idx * 16);
        ...
        writeMagnet(
          bufs,
          idx,
          magnetKey === socketKey(entity.id, socket.id, false) ? magnet : null
        );
  ...
  const resolveSourceSocketType = (
    ...
    const cacheKey = connectedSocketKey(
      connectionDraft.source.entityId,
      connectionDraft.source.socketId,
      connectionDraft.source.isInput
    );
        ...
        for (const entityId of movedIds) {
          ...
          if (!range) {
            ...
            if (arriving) {
              ...
              const h =
                arriving.height ?? getEntitySocketLayout(arriving, socketLayout).computedHeight;
              if (
