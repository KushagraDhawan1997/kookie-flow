...
import type { Entity, EdgeType, SocketType, EdgeMarker, EdgeMarkerType } from '../types/index';
...
export function unpackEdgeFlags(flags: number): {
  animated: boolean;
  selected: boolean;
  invalid: boolean;
} {
  ...
  declareOn(attribute: THREE.BufferAttribute, perVertex: number): boolean {
    ...
    for (let i = 0; i < this.count; i++) {
      attribute.addUpdateRange(
        this.starts[i] * perVertex,
        (this.ends[i] - this.starts[i]) * perVertex
      );
    }
    return true;
  }
...
export function Edges({
   ...
   */
  const socketIndexMapRef = useRef<Map<string, EntitySocketIndex>>(new Map());
  ...
  useFrame(({ size, clock }) => {
    ...
    if (
      ...
      !colorDirtyRef.current &&
      !layerDirtyRef.current
    )
      return;
...

    {
      for (let i = 0; i < edges.length; i++) {
        ...
        if (sourceSocketInfo) {
          sourceYOffset = getSocketYOffset(
            sourceEntity,
            sourceSocketInfo.index,
            false,
            socketLayout
          );
        }
        ...
        if (targetSocketInfo) {
          targetYOffset = getSocketYOffset(
            targetEntity,
            targetSocketInfo.index,
            true,
            socketLayout
          );
        }
        ...
        const cx1p = targetIsReroute
          ? targetEntity.position.x
          : getSocketWorldX(targetEntity, true);
