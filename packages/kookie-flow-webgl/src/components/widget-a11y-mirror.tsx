...
import type { Entity, SocketType } from '../types/index';
...
export function WidgetA11yMirror({
  ...
  useLayoutEffect(() => {
    const recompute = () => {
      ...
      const next = entity ? listEntityWidgets(entity, socketTypes, s.connectedSockets) : NO_ENTRIES;
  ...
  const handleChange = useCallback(
    (e: React.FormEvent<HTMLDivElement>) => {
      ...
      if (!entry) return;
      if (
        (entry.config.type === 'checkbox' || entry.config.type === 'switch') &&
        el instanceof HTMLInputElement
      ) {
...
function MirrorControlElement({
  ...
  switch (config.type) {
    ...
    case 'seed':
      return (
        <input {...common} ref={onMount} type="number" min={config.min} max={config.max} step={1} />
      );
    case 'vector':
