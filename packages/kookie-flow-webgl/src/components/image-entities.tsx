...
import type { ImageEntityData, EntityChange } from '../types/index';
...
function colorizeMipLevels(gl: THREE.WebGLRenderer, texture: THREE.Texture): void {
...
export function ImageEntities({ maxImageTextureSize, onEntitiesChange }: ImageEntitiesProps) {
  ...
  const [imageEntityIds, setImageEntityIds] = useState<string[]>(() => {
    return store
      .getState()
      .entities.filter((e) => e.type === 'image')
      .map((e) => e.id);
  });
  ...
  const texManager = useMemo(() => {
    return new ImageTextureManager(() => {
      fullDirtyRef.current = true;
    }, maxImageTextureSize);
  }, [maxImageTextureSize]);
  ...
  const placeholderMat = useMemo(() => createPlaceholderMaterial(surfaceColor), [surfaceColor]);
