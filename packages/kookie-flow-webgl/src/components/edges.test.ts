    ...
    for (const animated of [false, true]) {
      for (const selected of [false, true]) {
        for (const invalid of [false, true]) {
          ...
          expect(Math.sign(packVertexSide(-1, flags))).toBe(-1);
          expect(unpackEdgeFlags(Math.abs(packVertexSide(-1, flags)) - 1)).toEqual({
            animated,
            selected,
            invalid,
          });
...
describe('flagsToClearAfterEdgePass', () => {
  ...
  it('a rebuild raised during a partial pass survives into the next plan', () => {
    ...
    expect(
      planEdgeUpdate({ geometry: kept, position: true, color: false, layer: false }).geometry
    ).toBe('full');
  });
});
