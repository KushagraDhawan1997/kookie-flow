...
import { useResolvedStyle, useSocketLayout } from '../contexts/index';
...
export function EntitySelection() {
  ...
  const outlineMaterial = useMemo(() => {
    return new THREE.ShaderMaterial({
      uniforms: {
        uSelectedColor: {
          value: new THREE.Color(selectedColor[0], selectedColor[1], selectedColor[2]),
        },
  ...
  const attachOutline = useCallback(
    (mesh: THREE.InstancedMesh | null) => {
      ...
      outlineBuffers.outlineWidthAttr = new THREE.InstancedBufferAttribute(
        outlineBuffers.outlineWidths,
        1
      );
  ...
  const handleFillColor = useMemo(
    () => resolveColor(THEME_COLORS.entitySelection.handleFill, tokens),
    [tokens]
  );
  const handleBorderColor = useMemo(
    () => resolveColor(THEME_COLORS.entitySelection.handleBorder, tokens),
    [tokens]
  );
  ...
  const handleMaterial = useMemo(() => {
    return new THREE.ShaderMaterial({
      uniforms: {
        uFillColor: {
          value: new THREE.Color(handleFillColor[0], handleFillColor[1], handleFillColor[2]),
        },
        uBorderColor: {
          value: new THREE.Color(handleBorderColor[0], handleBorderColor[1], handleBorderColor[2]),
        },
        uZoom: { value: 1.0 },
  ...
  useEffect(() => {
    ...
    const unsubConnection = store.subscribe(
      (state) => state.connectionDraft !== null,
      markHandleDirty
    );
  ...
  useFrame(({ size }) => {
    ...
    if (outlineMesh && outlineInitializedRef.current && outlineDirtyRef.current) {
      const { viewport, selectedEntityIds, hoveredEntityId, hiddenEntityIds, entityMap } =
        store.getState();
       ...
       */
      const contributes = (entity: import('../types/index').Entity): boolean => {
      ...
      const writeOutline = (entity: import('../types/index').Entity, isSelected: boolean) => {
        ...
        outlineBuffers.outlineWidths[outlineCount] = isSelected
          ? selOutlineWidth
          : hoverOutlineWidth;
      ...
      if (
        outlineBuffers.sizeAttr &&
        outlineBuffers.typeAttr &&
        outlineBuffers.outlineWidthAttr &&
        outlineBuffers.paddingAttr
      ) {
    ...
    if (handleMesh && handleInitializedRef.current && handleDirtyRef.current) {
      ...
      const showHandles =
        ...
        selectedEntityIds.has(hoveredEntityId);
      ...
      if (
        ...
        entity.resizable !== false &&
        handleCount + 4 <= maxHandles
      ) {
        const width = entity.width ?? DEFAULT_ENTITY_WIDTH;
        const entityLayout = getEntitySocketLayout(entity, socketLayout);
        const height = entity.height ?? entityLayout.computedHeight;
        ...
        // Frustum culling
        const offscreen =
          x + width < viewLeft || x > viewRight || y + height < viewTop || y > viewBottom;
