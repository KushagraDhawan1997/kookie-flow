...
import type { TextEntityData, EntityChange } from '../types/index';
import {
  DEFAULT_TEXT_WIDTH,
  DEFAULT_TEXT_HEIGHT,
...
import { NO_WRAP_WIDTH } from '@kushagradhawan/kookie-flow-core/internal/utils/text-texture';
import {
...
export function TextEntities({ onEntitiesChange }: TextEntitiesProps) {
  ...
  useFrame(({ size }) => {
    ...
    for (let i = 0; i < entities.length; i++) {
      ...
      if (entity.type !== 'text' && !isComment) continue;
      if (hiddenEntityIds.has(entity.id)) continue;

      let w = entity.width ?? (isComment ? 200 : DEFAULT_TEXT_WIDTH);
      let h = entity.height ?? (isComment ? 100 : DEFAULT_TEXT_HEIGHT);
      const data = entity.data as TextEntityData;

      const fontSize = data.fontSize ?? (isComment ? 14 : DEFAULT_TEXT_FONT_SIZE);
      const lineHeight = data.lineHeight ?? (isComment ? 1.4 : DEFAULT_TEXT_LINE_HEIGHT);
      const padding = isComment ? 12 : DEFAULT_TEXT_PADDING;
      const letterSpacing = data.letterSpacing ?? 0;
      const textAlign = data.textAlign ?? 'left';
      const sizingMode = isComment ? 'fixed' : (data.sizingMode ?? DEFAULT_TEXT_SIZING_MODE);
      // Use live editing content when this entity is being edited
      const content =
        editingEntityId === entity.id && editingContent !== null
          ? editingContent
          : (data.content ?? '');
      ...
      if (sizingMode === 'auto-width') {
        ...
        expectedH = Math.max(measurement.height + 2 * padding, fontSize * lineHeight + 2 * padding);
      } else {
      ...
      const textColor = isComment
        ? commentPaint(entity, tokens, scope).text
        : (data.textColor ?? primaryTextColor);
      ...
      if (!content || !content.trim()) {
        ...
        targetEntries.push({
          ...
          position: [
            ...
            entityDepth(entity.id, stackOrder, selectedForDepth, DEPTH_LAYER.label),
          ],
          fontSize,
          lineHeight,
          textAlign: 'left',
      ...
      const clipHeight =
        sizingMode === 'fixed' && h < measurement.height + 2 * padding
          ? h - 2 * padding
          : undefined;
