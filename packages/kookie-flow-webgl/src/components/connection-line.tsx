...
import { SDF_GLSL } from '../gl/index';
import { SOCKET_RADIUS } from '@kushagradhawan/kookie-flow-core/internal/core/constants';
import {
  EDGE_LEADER,
  EDGE_SOCKET_RIM,
  bezierControlOffset,
} from '@kushagradhawan/kookie-flow-core/internal/utils/edge-curve';
import type { SocketType } from '../types/index';
 ...
 */
export function ConnectionLine({ socketTypes = DEFAULT_SOCKET_TYPES }: ConnectionLineProps) {
  ...
  useFrame(({ clock }, delta) => {
    ...
    if (socketCacheRef.current?.key === cacheKey) {
      ...
      socketIndex = sourceSockets.findIndex((s) => s.id === connectionDraft.source.socketId);
      if (socketIndex === -1) {
        mesh.visible = false;
        return;
      }
    ...
    if (!connectionDraft.isValid) {
      ...
      const typeConfig = socketTypes[socket.type] ??
        socketTypes.any ?? { color: fallbackSocketColor, name: 'Any' };
    ...
    (bridgeMaterial.uniforms.uColor.value as THREE.Color).copy(
