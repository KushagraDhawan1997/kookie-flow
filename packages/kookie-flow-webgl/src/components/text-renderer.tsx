...
import type { EdgeType, EdgeLabelConfig, SocketType } from '../types/index';
import {
  getEdgePointAtT,
  type SocketIndexMap,
...
import { isSelfDrawn } from '@kushagradhawan/kookie-flow-core/internal/utils/entity-kind';
import {
  entityDepth,
  DEPTH_LAYER,
...
export function MultiWeightTextRenderer({
    ...
    for (const n of entities) {
      if (n.inputs) {
        for (let i = 0; i < n.inputs.length; i++) {
          const s = n.inputs[i];
          socketIndexMapRef.current.set(connectedSocketKey(n.id, s.id, true), {
            index: i,
            socket: s,
          });
        }
      ...
      if (n.outputs) {
        for (let i = 0; i < n.outputs.length; i++) {
          const s = n.outputs[i];
          socketIndexMapRef.current.set(connectedSocketKey(n.id, s.id, false), {
            index: i,
            socket: s,
          });
        }
    ...
    (
        ...
        for (let vi = 0; vi < visibleCount; vi++) {
          ...
          if (entity.outputs) {
            for (let i = 0; i < entity.outputs.length; i++) {
              ...
              const socketY =
                entity.position.y +
                (cachedPos?.labelY ?? socketLayout.marginTop + socketLayout.rowHeight / 2) +
                centerOffset;
              ...
              regular.push({
                text: truncatedName,
                position: [
                  entity.position.x + width - contentInset,
                  textY,
                  entityDepth(entity.id, stackOrder, selectedEntityIds, DEPTH_LAYER.label),
                ],
                fontSize: 12,
      ...
      if (
        ...
        for (let vi = 0; vi < visibleCount; vi++) {
          ...
          const depth = entityDepth(entity.id, stackOrder, selectedEntityIds, DEPTH_LAYER.label);
          ...
          for (let i = 0; i < inputs.length; i++) {
            ...
            for (let t = 0; t < count; t++) {
              ...
              if (placed.multiline) {
                ...
                const lines = wrapTextMSDF(
                  placed.text,
                  placed.maxWidth / glyphScale,
                  regularGlyphMap,
                  regularKerningMap
                );
                ...
                for (let l = 0; l < shown; l++) {
                  ...
                  if (l === shown - 1 && lines.length > shown) {
                    ...
                    line =
                      measureText(more, regularGlyphMap, regularKerningMap) * glyphScale <=
                      placed.maxWidth
                        ? more
                        : truncateText(
                            more,
              ...
              regular.push({
                ...
                position: [
                  placed.x,
                  box.y +
                    Math.min(box.height, socketLayout.widgetHeight) / 2 -
                    (widgetFont * 7) / 12,
                  depth,
                ],
                fontSize: widgetFont,
  ...
  useEffect(() => {
    ...
    reindexConnected();
    const unsubConnected = store.subscribe(
  ...
  return (
    <>
      <TextWeightRenderer fontData={regularFont} entriesRef={regularEntriesRef} />
      {/*
      ...
      */}
      {semiboldFont && (
        <TextWeightRenderer fontData={semiboldFont} entriesRef={semiboldEntriesRef} />
      )}
    </>
  );
}
...
export function TextRenderer({ fontMetrics, atlasTexture, ...props }: TextRendererProps) {
  ...
  return <MultiWeightTextRenderer regularFont={fontData} {...props} />;
}
