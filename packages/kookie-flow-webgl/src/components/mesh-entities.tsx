...
import type { MeshEntityData, EntityChange } from '../types/index';
  ...
  useFrame(({ gl, size, viewport: glViewport }, delta) => {
    ...
    for (let i = 0; i < ids.length; i++) {
      ...
      if (!target) {
        ...
        if (
          targetsRef.current.size >= MAX_MESH_TARGETS &&
      ...
      if (mat) {
        ...
        const presence = easeChromePresence(
          chromePresenceRef.current,
          entity.id,
          wantsChrome,
          delta
        );
