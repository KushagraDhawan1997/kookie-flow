...
import { entitySocketKey } from '@kushagradhawan/kookie-flow-core/internal/utils/socket-key';
...
import type {
  GlyphMap,
  KerningMap,
} from '@kushagradhawan/kookie-flow-core/internal/utils/text-layout';
import {
...
import { diffGraph } from '@kushagradhawan/kookie-flow-core/internal/core/graph-diff';
import {
  findAlignment,
  type AlignRect,
} from '@kushagradhawan/kookie-flow-core/internal/utils/alignment';
...
import type { SocketEntry } from '@kushagradhawan/kookie-flow-core/internal/core/spatial';
import {
  getSocketPosition,
  socketKey,
  screenToWorld,
...
import { widgetKey } from '@kushagradhawan/kookie-flow-core/internal/utils/widget-values';
import {
  popoverLayoutFor,
  popoverContains,
  selectRowAt,
...
} from '@kushagradhawan/kookie-flow-core/internal/utils/popover-layout';
import { hsvToHex } from '@kushagradhawan/kookie-flow-core/internal/utils/hsv';
import { measureText } from '@kushagradhawan/kookie-flow-core/internal/utils/text-layout';
import { motionNow } from '../gl/index';
...
import {
  boundsFromCorners,
  getEntityBounds,
} from '@kushagradhawan/kookie-flow-core/internal/core/spatial';
...
import { topmostEntityId } from '@kushagradhawan/kookie-flow-core/internal/utils/entity-depth';
import { locksAspectByDefault } from '@kushagradhawan/kookie-flow-core/internal/utils/entity-kind';
        ...
        >
          ...
          >
            <FlowCanvas
              ...
              onEntitiesChange={onEntitiesChange}
            />
            <DOMLayer entityTypes={entityTypes} onEntitiesChange={onEntitiesChange}>
              {children}
            </DOMLayer>
    ...
    useImperativeHandle(
      ...
      () => ({
        ...
        zoomIn: (step = 0.25) => {
          ...
          state.setViewport({
            ...state.viewport,
            zoom: Math.max(minZoom, Math.min(maxZoom, state.viewport.zoom + step)),
          });
        },
...
function InputHandler({
  ...
  const openWidgetEdit = useCallback(
    (hit: WidgetHit | null) => {
      ...
      store
        .getState()
        .setEditingWidgetKey(hit ? widgetKey(hit.entityId, hit.socketId) : null, hit?.part);
    },
    [store]
  );
  ...
  const applyCursor = useCallback(() => {
    ...
    el.style.cursor =
      base !== 'default'
        ? base
        : widgetCursorRef.current
          ? widgetCursorRef.current
          : (chromeCursorRef.current ?? base);
  }, []);
  ...
  const runAutoScroll = useCallback(() => {
    ...
    const rectSource =
      dragState.current?.containerRect ?? containerRef.current?.getBoundingClientRect();
    if (!rectSource) {
      autoScrollRef.current.active = false;
      return;
    }
      ...
      switch (entity.type) {
        ...
        default: {
          ...
          return {
            ...
            minHeight: Math.max(MIN_ENTITY_HEIGHT, layout.computedHeight),
          };
        }
      }
    },
  ...
  const getResizeHandleAt = useCallback(
    (worldX: number, worldY: number): { entityId: string; handle: ResizeHandle } | null => {
    ...
    (entityId: string, worldX: number, worldY: number): ChromeCursor | null => {
      ...
      if (preview && (preview.controls ?? true)) {
        ...
        if (
          ...
          worldX >= band.x &&
          worldX <= band.x + band.width &&
          worldY >= band.y &&
          worldY <= band.y + band.height
        ) {
  ...
  const handlePointerDown = useCallback(
    (e: ReactPointerEvent) => {
        ...
        if (e.button !== 0) {
          ...
          if (layout && popoverContains(layout, worldPos.x, worldPos.y)) {
            if (layout.kind === 'select') {
              const row = selectRowAt(
                layout,
                store.getState().popoverScroll,
                worldPos.x,
                worldPos.y
              );
      ...
      if (e.button === 0) {
            ...
            entities: [
              {
                ...
                data: {
                  ...
                  ...(penStyleRef.current?.strokeColor
                    ? { strokeColor: penStyleRef.current.strokeColor }
                    : {}),
                } as DrawEntityData,
        ...
        const socket = getSocketAtPositionFast(worldPos, socketQuadtree, socketLayout);
        ...
        const clickedEntity =
          entityMap.get(
            topmostEntityId(
              ...
              store.getState().stackOrder,
              store.getState().selectedEntityIds
            ) ?? ''
          ) ?? null;
            ...
            if (hit.config.type === 'seed') {
              if (isOnSeedButton(hit.box, worldPos.x)) {
                ...
                emitWidgetChange(
                  hit.entityId,
                  hit.socketId,
                  randomSeed(hit.config.min, hit.config.max)
                );
                return;
              }
        ...
        if (
          clickedEntity &&
          (clickedEntity.type === 'image' ||
            clickedEntity.type === 'video' ||
            clickedEntity.type === 'mesh')
        ) {
        ...
        if (clickedEntity?.preview && (clickedEntity.preview.controls ?? true)) {
          ...
          if (
            ...
            worldPos.x >= band.x &&
            worldPos.x <= band.x + band.width &&
            worldPos.y >= band.y &&
            worldPos.y <= band.y + band.height
          ) {
            ...
            if (source.kind !== 'none' && hitExpandButton(lx, ly, band.width, band.height)) {
              ...
              if (source.kind === 'video') {
                ...
                const orbit = hasOrbit(store, clickedEntity.id)
                  ? getOrbit(store, clickedEntity.id)
                  : orbitFromDirection(
                      BAND_MODEL_DIRECTION.x,
                      BAND_MODEL_DIRECTION.y,
                      BAND_MODEL_DIRECTION.z
                    );
          ...
          if (editingId === clickedEntity.id && clickedEntity.type === 'text') {
            ...
            if (font && font.glyphMap.size > 0) {
              ...
              textSelectEntityRef.current = {
                x: clickedEntity.position.x,
                y: clickedEntity.position.y,
                pad,
              };
            }
    ...
    (e: ReactPointerEvent) => {
      ...
      if (scrub) {
        if ((e.buttons & 1) === 0) {
          ...
          videoOps(store, scrub.surface)?.seek(
            scrub.entityId,
            seekPositionAt(world.x - scrub.left, scrub.width)
          );
          return;
        }
      }
      ...
      if (connectionDraft && primaryButtonDown) {
        ...
        const hoveredSocket = getSocketAtPositionFast(worldPos, connSocketQuadtree, socketLayout);
        ...
        if (hoveredSocket) {
          ...
          if (isTypeCompatible && !allowCycles) {
            ...
            const sourceNodeId = isSourceInput
              ? hoveredSocket.entityId
              : connectionDraft.source.entityId;
            const targetNodeId = isSourceInput
              ? connectionDraft.source.entityId
              : hoveredSocket.entityId;
      ...
      if (resizeState.current && primaryButtonDown) {
        ...
        if (h === 'w' || h === 'nw' || h === 'sw') {
          newW = rs.initialBounds.width - dx;
          newX = rs.initialBounds.x + dx;
        }
        if (h === 's' || h === 'se' || h === 'sw') newH = rs.initialBounds.height + dy;
        if (h === 'n' || h === 'ne' || h === 'nw') {
          newH = rs.initialBounds.height - dy;
          newY = rs.initialBounds.y + dy;
        }
          ...
          if (sizingMode === 'auto-height') {
            ...
            if (resizeFont && resizeFont.glyphMap.size > 0) {
              newH = calculateTextAutoHeightMSDF(
                ...
                style,
                newW,
                resizeFont.metrics.info.size,
                resizeFont.glyphMap,
                resizeFont.kerningMap
              );
            } else {
        ...
        if (!snapToGrid && helperLines) {
          ...
          if (primary) {
            ...
            for (const id of s.quadtree.queryRange(alignRange)) {
              ...
              slot.height =
                candidate.height ?? getEntitySocketLayout(candidate, socketLayout).computedHeight;
              candidateCount++;
            }
      ...
      if (!pointerDownPos.current) {
        ...
        const {
          ...
          hoveredEntityId,
          quadtree,
          socketQuadtree: hoverSocketQuadtree,
        } = store.getState();
        const worldPos = screenToWorld({ x: screenX, y: screenY }, viewport);
        ...
        setWidgetCursor(
          hoveredWidgetSocketId === null || newHoveredId === null
            ? false
            : inputWidgetType(
                  store.getState().entityMap.get(newHoveredId),
                  hoveredWidgetSocketId,
                  socketTypes
                ) === 'vector'
              ? 'ew-resize'
              : true
    ...
    (e: ReactPointerEvent) => {
      ...
      if (releasedVector && releasedVector.pointerId === e.pointerId) {
        ...
        if (!releasedVector.moved && e.type === 'pointerup') {
          ...
          const box = readPartBoxInto(
            { x: 0, y: 0, width: 0, height: 0 },
            hit.box,
            vectorDimensions(hit.config),
            part
          );
      ...
      if (widgetDragRef.current && widgetDragRef.current.pointerId === e.pointerId) {
        ...
        store
          ...
          .setHoveredWidget(
            stillOn
              ? { entityId: releasedDrag.hit.entityId, socketId: releasedDrag.hit.socketId }
              : null
          );
      ...
      if (pendingSelect && pendingSelect.pointerId === e.pointerId) {
        ...
        if (e.type === 'pointerup' && travelled <= DRAG_THRESHOLD)
          openWidgetPopover(pendingSelect.hit);
      ...
      if (connectionDraft) {
        ...
        const dropTarget =
          e.type === 'pointerup' ? (hoveredSocketId ?? magnetTargetRef.current) : null;
        magnet.end();
        magnetTargetRef.current = null;
        ...
        if (dropTarget) {
          ...
          if (isValid) {
            ...
            const connection: Connection = {
              ...
              sourceSocket: isSourceInput ? dropTarget.socketId : connectionDraft.source.socketId,
              target: isSourceInput ? connectionDraft.source.entityId : dropTarget.entityId,
              targetSocket: isSourceInput ? connectionDraft.source.socketId : dropTarget.socketId,
          ...
          const changes: EntityChange[] = [
            {
              ...
              id: rs.entityId,
              dimensions: {
                width: entity.width ?? rs.initialBounds.width,
                height: entity.height ?? rs.initialBounds.height,
              },
            },
          ];
          // Handles that move the entity origin (N/NW/NE/W/SW) also change position
          if (
            entity.position.x !== rs.initialBounds.x ||
            entity.position.y !== rs.initialBounds.y
          ) {
      ...
      if (pointerDownPos.current && !hasDragged.current && e.button === 0) {
        ...
        if (clickedEntity) {
          ...
          if (currentEditingId === clickedEntity.id && clickedEntity.type === 'text') {
            const data = clickedEntity.data as TextEntityData;
            const font = resolveFontForWeight(fontContext, data.fontWeight ?? 400);
            if (font && font.glyphMap.size > 0) {
              const content = store.getState().editingContent ?? data.content ?? '';
  ...
  const onEntitiesChangeRef = useRef(onEntitiesChange);
  useEffect(
    () => bindHistoryOwner(containerRef.current, () => onEntitiesChangeRef.current),
    [containerRef]
  );
  const onEdgesChangeRef = useRef(onEdgesChange);
  ...
  useEffect(() => {
    ...
    const jumpEntityCursor = (direction: 1 | -1) => {
      const s = store.getState();
      placeCursor(stepEntityCursor(s.entities, null, direction, (id) => s.hiddenEntityIds.has(id)));
    };
  ...
  const handleTouchStart = useCallback(
    (e: React.TouchEvent) => {
      ...
      if (
        touchState.current.touches.size === 2 &&
        !widgetDragRef.current &&
        !vectorDragRef.current
      ) {
  ...
  const baseCursor = penActive
    ...
    : isResizing
      ? resizeState.current
        ? RESIZE_CURSORS[resizeState.current.handle]
        : 'default'
...
interface FlowCanvasProps {
  ...
  showGrid: boolean;
  showStats: boolean;
  defaultEdgeType: import('../types/index').EdgeType;
  socketTypes: Record<string, SocketType>;
  showSocketLabels: boolean;
  showEdgeLabels: boolean;
  maxImageTextureSize?: number;
...
function FlowSync({ entities, edges, socketTypes }: FlowSyncProps) {
  ...
  useEffect(() => {
    const syncEdges = (incoming: Edge[]) => {
      store
        .getState()
        .setEdges(resolveEdgeValidity(incoming, store.getState().entityMap, socketTypes));
    };
    syncEdges(edges);
    ...
    return store.subscribe(
      (state) => state.entityMap,
      () => syncEdges(store.getState().edges)
    );
