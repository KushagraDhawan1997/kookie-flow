...
export function PreviewEntities() {
  ...
  function writeChrome(
    ...
    const download =
      source.kind !== 'bitmap' && source.kind !== 'none' && fitsDownload(band.width, band.height)
        ? presence
        : 0;
    setMediaChrome(
      mat,
      bar && presence > 0.001 ? 1 : 0,
      progress,
      playing,
      bar ? presence : 0,
      presence,
  ...
  function chromePass(delta: number): void {
    ...
    const step = (id: string) => {
      ...
      if (writeChrome(entity, band, mat, hold.source, hoveredEntityId, pointerWorld, delta))
        fading = true;
    };

    const hovered =
      hoveredEntityId && entityMap.get(hoveredEntityId)?.preview ? hoveredEntityId : null;
  ...
  useFrame(({ gl, size, viewport: glViewport }, delta) => {
    ...
    for (let i = 0; i < ids.length; i++) {
      ...
      if (
        x + w < viewLeft - CULL_PADDING ||
        x > viewRight + CULL_PADDING ||
        y + h < viewTop - CULL_PADDING ||
        y > viewBottom + CULL_PADDING
      ) {
        quad.visible = false;
        continue;
      }
      ...
      if (source.kind === 'mesh') {
        ...
        if (!target) {
          ...
          if (
            targetsRef.current.size >= MAX_PREVIEW_TARGETS &&
            !reclaimColdestTarget(entity.id, frame)
          ) {
            quad.visible = false;
            continue;
          }
        ...
        if (writeChrome(entity, band, mat, source, hoveredEntityId, pointerWorld, delta))
          chromeFading = true;
        quad.material = mat;
        continue;
      }
    ...
    chromeActiveRef.current =
      chromeFading ||
      (hoveredEntityId !== null && entityMap.get(hoveredEntityId)?.preview !== undefined);
