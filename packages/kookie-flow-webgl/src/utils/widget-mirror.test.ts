...
describe('listEntityWidgets only accepts GPU widget configuration', () => {
  it('rejects obsolete inline DOM components received from untyped callers', () => {
    const entity = node([
      // @ts-expect-error The data-only socket API rejects React components too.
      { id: 'a', name: 'A', type: 'string', widget: { component: Stub } },
    ]);
    expect(() => listEntityWidgets(entity, socketTypes, new Set())).toThrow('Inline DOM widgets');
  });
});
