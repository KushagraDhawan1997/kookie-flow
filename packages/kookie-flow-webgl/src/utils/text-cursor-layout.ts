...
export function getSelectionRects(
  ...
  const endPosIdx =
    hi >= table.contentLength ? positions.length : (contentToPos[hi] ?? positions.length);
  ...
  const endLine =
    endPosIdx < positions.length ? positions[endPosIdx].line : positions[positions.length - 1].line;
 ...
 */
export function visualLineForOffset(contentOffset: number, table: CharPositionTable): number {
  ...
  if (contentOffset > 0 && contentOffset < table.contentLength) {
    ...
    if (
      posIdx !== undefined &&
      prevPosIdx !== undefined &&
      posIdx < table.positions.length &&
      prevPosIdx < table.positions.length
    ) {
...
export function lineColumnToContentOffset(
  ...
  if (posIdx >= positions.length) {
    ...
    return last !== undefined
      ? last + positions[positions.length - 1].codeUnitLength
      : table.contentLength;
  }
 ...
 */
export function getWordBoundary(offset: number, content: string): { start: number; end: number } {
