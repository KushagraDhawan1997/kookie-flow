...
describe('the expand button', () => {
  ...
  it('sits in the top-right corner', () => {
    expect(
      hitExpandButton(W - inset - EXPAND_BUTTON_SIZE / 2, inset + EXPAND_BUTTON_SIZE / 2, W, H)
    ).toBe(true);
  });
...
describe('a downloaded file name', () => {
  it('keeps the URL file name, without its query', () => {
    expect(downloadFileName('https://cdn.test/a/lighthouse.png?v=2', 'image', '')).toBe(
      'lighthouse.png'
    );
  });

  it('falls back to the kind and the content type', () => {
    expect(downloadFileName('https://cdn.test/files/abc123', 'image', 'image/jpeg')).toBe(
      'image.jpg'
    );
...
describe('the cursor over media chrome', () => {
  ...
  it('tells the two regions of a model apart: the strip moves the node, the body turns the model', () => {
    expect(
      mediaEntityCursor('mesh', MW / 2, MESH_DRAG_STRIP_HEIGHT / 2, MW, MH, true, true, true)
    ).toBe('move');
