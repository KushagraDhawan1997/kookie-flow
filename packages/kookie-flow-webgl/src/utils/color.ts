export {
  ...
  type RGBColor,
  type RGBAColor,
} from '@kushagradhawan/kookie-flow-core/internal/utils/hex-color';
import {
  hexToRGB,
  hexToRGBA,
  type RGBColor,
...
import { themeRoot } from './theme-root'; // [0-1, 0-1, 0-1, 0-1]
...
function getColorProbe(): HTMLSpanElement | null {
  ...
  (probeHost ?? themeRoot())?.appendChild(colorProbe);
  colorProbeDepth++;
  return colorProbe;
}
...
const COLOR_P3_RE =
  /color\(display-p3\s+(-?[\d.eE+-]+)\s+(-?[\d.eE+-]+)\s+(-?[\d.eE+-]+)(?:\s*\/\s*([\d.eE+-]+))?/;
...
function displayP3ToSRGB(r: number, g: number, b: number): RGBColor {
  ...
  return [
    ...
    clamp01(toEncoded(-0.0196375546 * lr - 0.0786360655 * lg + 1.09827362 * lb)),
  ];
}
...
function parseRGBString(color: string): RGBColor {
  ...
  if (p3Match) {
    return displayP3ToSRGB(parseFloat(p3Match[1]), parseFloat(p3Match[2]), parseFloat(p3Match[3]));
  }
  ...
  const modernMatch = color.match(/rgba?\(\s*(\d+(?:\.\d+)?)\s+(\d+(?:\.\d+)?)\s+(\d+(?:\.\d+)?)/);
  if (modernMatch) {
    return [
...
/** Resolve CSS colours in one editor's scope, restoring the previous host even on failure. */
export function withColorScope<T>(host: Element | null, fn: () => T): T {
  const previous = probeHost;
  probeHost = host;
  try {
    return withProbes(fn);
  } finally {
    probeHost = previous;
  }
}
