...
import { MOTION_SPEED } from '../gl/index';
...
export function hitExpandButton(
  localX: number,
  localY: number,
  width: number,
  height: number
...
export function hitDownloadButton(
  localX: number,
  localY: number,
  width: number,
  height: number
...
export function isOnVideoControls(
  localX: number,
  localY: number,
  width: number,
  height: number
...
export function mediaEntityCursor(
  ...
  if (
    hasSrc &&
    controls &&
    (hitExpandButton(localX, localY, width, height) ||
      hitDownloadButton(localX, localY, width, height))
  ) {
    return 'pointer';
  }
  if (kind === 'video')
    return controls && isOnVideoControls(localX, localY, width, height) ? 'pointer' : null;
...
export function bandCursor(
  ...
  if (source !== 'none' && source !== 'bitmap' && hitDownloadButton(localX, localY, width, height))
    return 'pointer';
  if (source === 'video')
    return isOnVideoControls(localX, localY, width, height) ? 'pointer' : null;
