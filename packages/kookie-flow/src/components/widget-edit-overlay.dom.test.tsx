...
describe('the borrowed element is transparent and sits where the glyphs were', () => {
  ...
  it('keeps a select clear of the GL chevron and draws no arrow of its own', () => {
    const { container } = mount(hitFor(selectConfig, 'a'));
    const el = selectIn(container);
    expect(el.style.background).toBe('transparent');
    expect(el.style.paddingRight).toBe('20px');
    expect(el.style.paddingLeft).toBe(`${PAD}px`);
    expect(el.style.appearance).toBe('none');
  });

  it('makes a colour input invisible: the swatch is GL\'s, the picker is the DOM\'s', () => {
    const { container } = mount(hitFor({ type: 'color' }, '#ff0000'));
