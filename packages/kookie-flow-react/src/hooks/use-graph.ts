...
import { historyOwner } from '@kushagradhawan/kookie-flow-webgl/internal/hooks/history-owner';
import type {
  Entity,
  Edge,
  EntityChange,
  ...
  Connection,
  TextEntityData,
} from '@kushagradhawan/kookie-flow-core/types';
import { resizableForSizingMode } from '@kushagradhawan/kookie-flow-core/internal/utils/text-texture';
import {
...
export function useGraph(options: UseGraphOptions = {}): UseGraphReturn {
  ...
  const historyShortcuts = (typeof history === 'object' ? history.shortcuts : undefined) ?? true;
      ...
      if (
        graphRef.current.edges.some(
          (edge) =>
            ...
            edge.sourceSocket === sourceSocket &&
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      if (
        historyContainerRef
          ...
          : !container || historyOwner(container) !== onEntitiesChange
      )
        return;
      e.preventDefault();
  ...
  const getEntity = useCallback((id: string) => entities.find((n) => n.id === id), [entities]);
