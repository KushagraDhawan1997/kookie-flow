...
import type { Entity } from '@kushagradhawan/kookie-flow-core/types';
...
const entities: Entity[] = [{ id: 'a', type: 'default', position: { x: 0, y: 0 }, data: {} }];
...
describe('undo through useGraph', () => {
  it('deleting a wired node is one step, though the wire and the node arrive as two batches', () => {
    ...
    act(() => {
      g.api().onEdgesChange([{ type: 'add', edge: { id: 'e', source: 'a', target: 'b' } }]);
    });
